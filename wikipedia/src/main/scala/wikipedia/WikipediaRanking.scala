...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd
      .aggregate(0)(seqOp = (sum, article) => sum + (if (article.mentionsLanguage(lang)) 1 else 0),
                    combOp = _ + _)
  ...
  def makeIndex(langs: List[String],
    ...
    rdd
      .flatMap(wa => langs.filter(wa.mentionsLanguage).map(_ -> wa))
      .groupBy { case (lang, _) => lang }
      .mapValues(_.map { case (_, article) => article })
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.mapValues(_.size).sortBy { case (_, count) => -count }.collect().toList
